...
create_server_node() {
    ...
    multipass launch --name $node_name --cpus $SERVER_CPU --memory $SERVER_MEM --disk $SERVER_DISK --cloud-init $K3S_CONFIG_DIR/$CLUSTER_NAME-server-cloud-init.yaml
}

create_agent_node() {
    local node_name=$1
    echo "Creating Agent Node $node_name"
    multipass launch --name $node_name --cpus $AGENT_CPU --memory $AGENT_MEM --disk $AGENT_DISK --cloud-init $K3S_CONFIG_DIR/$CLUSTER_NAME-agent-cloud-init.yaml
}
